 ...
 * @typedef {import("./faustwasm2").FaustDspMeta} FaustDspMeta
 * @typedef {import("./faustwasm2").FaustMonoAudioWorkletNode} FaustMonoAudioWorkletNode
 * @typedef {import("./faustwasm2").FaustPolyAudioWorkletNode} FaustPolyAudioWorkletNode
 * @typedef {import("./faustwasm2").FaustMonoScriptProcessorNode} FaustMonoScriptProcessorNode
 * @typedef {import("./faustwasm2").FaustPolyScriptProcessorNode} FaustPolyScriptProcessorNode
...
const createFaustNode = async (audioContext, dspName = "template", voices = 0, sp = false, bufferSize = 512) => {
    ...
    const { FaustMonoDspGenerator, FaustPolyDspGenerator } = await import("./faustwasm2/index.js");

    // Load DSP metadata from JSON
    /** @type {FaustDspMeta} */
    const dspMeta = await (await fetch("./faust/mockDSP2.json")).json();

    // Compile the DSP module from WebAssembly binary data
    const dspModule = await WebAssembly.compileStreaming(await fetch("./faust/mockDSP2.wasm"));
 ...
 * 
 * @param {FaustMonoAudioWorkletNode} faustNode 
 */
